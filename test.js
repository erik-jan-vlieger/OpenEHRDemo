...
async function run() {
  ...
  const comp = JSON.parse(fs.readFileSync('test_wound.json'));
  const res = await fetch(`http://localhost:8080/ehrbase/rest/openehr/v1/ehr/${ehrId}/composition?format=FLAT&templateId=wound_assessment_sensire`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'Prefer': 'return=representation', 'Authorization': auth },
